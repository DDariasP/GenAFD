...
public class MyLexer extends Lexer implements MyConstants {
	...
	protected int transition(int state, char symbol) 
	{
		switch(state) 
		{
			...
			case 6:
				...
				else if(symbol >= '0' && symbol <= '9') return 6;
				else if(symbol == '_') return 6;
				else return -1;
                        case 7:
				if(symbol == ':') return 8;
			...
			case 18:
				if(symbol == 'n' || symbol == 't' || symbol == 'b') return 19;
				else if(symbol == 'r' || symbol == 'f' || symbol == '\\') return 19;
				else if(symbol == '\'' || symbol == '\"') return 19;
				else return -1;
			case 19:
				if(symbol == '\'') return 20;
				else return -1;
	...
	protected Token getToken(int state, String lexeme, int row, int column) 
	{
		switch(state) 
		{
			...
			case 16: return new Token(HOOK, lexeme, row, column);
			case 20: return new Token(SYMBOL, lexeme, row, column);
			default: return null;
		}
	}
